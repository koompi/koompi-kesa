...
//! operations. Gated behind `KESA_PERF_TELEMETRY=1` for zero overhead in
...
impl SessionMetrics {
    ...
    pub fn summary(&self) -> String {
        if !self.enabled() {
            return "Session telemetry disabled (set KESA_PERF_TELEMETRY=1 to enable)".to_string();
        }
        let s = self.snapshot();
        format!(
...
///
/// On first call, reads `KESA_PERF_TELEMETRY` to decide whether collection
