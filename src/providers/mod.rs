...
fn vcr_client_if_enabled(base_url: &str) -> Result<Option<Client>> {
    ...
    if base_url_targets_loopback(base_url) && !crate::env::is_set("VCR_ALLOW_LOOPBACK") {
        return Ok(None);
    }

    let test_name = crate::env::var("VCR_TEST_NAME").unwrap_or_else(|| "pi_runtime".to_string());
