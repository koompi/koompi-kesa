//! Comprehensive environment health checker for `kesa doctor`.
//!
...
pub fn run_doctor(opts: &DoctorOptions<'_>) -> Result<DoctorReport> {
        ...
        .is_some_and(|set| set.contains(&CheckCategory::Extensions))
    {
        findings.push(
            ...
            .with_remediation(
                "Run `kesa doctor <path-to-extension>` to evaluate extension compatibility",
            ),
        );
    }
        ...
        PreflightVerdict::Fail => {
            findings.push(
                Finding::fail(cat, format!("Extension {ext_id}: incompatible"))
                    ...
                    .with_remediation(format!("Try: kesa doctor {path} --policy permissive")),
            );
        }
    }
