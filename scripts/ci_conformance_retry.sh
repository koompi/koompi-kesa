...
#
# Environment:
#   KESA_CONFORMANCE_MAX_RETRIES   Max retries (default: 1)
#   KESA_CONFORMANCE_RETRY_DELAY   Seconds between retries (default: 5)
#   KESA_CONFORMANCE_CLASSIFY_ONLY Set to 1 to classify without retrying
#   KESA_CONFORMANCE_CARGO_RUNNER  Cargo runner mode: rch | auto | local (default: rch)

set -euo pipefail

CARGO_RUNNER_REQUEST="${KESA_CONFORMANCE_CARGO_RUNNER:-rch}" # rch | auto | local
...
if [[ "$CARGO_RUNNER_REQUEST" != "rch" && "$CARGO_RUNNER_REQUEST" != "auto" && "$CARGO_RUNNER_REQUEST" != "local" ]]; then
    echo "Invalid KESA_CONFORMANCE_CARGO_RUNNER value: $CARGO_RUNNER_REQUEST (expected: rch|auto|local)" >&2
    exit 2
fi

if [[ "$CARGO_RUNNER_REQUEST" == "rch" ]]; then
    if ! command -v rch >/dev/null 2>&1; then
        echo "KESA_CONFORMANCE_CARGO_RUNNER=rch requested, but 'rch' is not available in PATH." >&2
        exit 2
    fi
...
MAX_RETRIES="${KESA_CONFORMANCE_MAX_RETRIES:-1}"
RETRY_DELAY="${KESA_CONFORMANCE_RETRY_DELAY:-5}"
CLASSIFY_ONLY="${KESA_CONFORMANCE_CLASSIFY_ONLY:-0}"
FLAKE_LOG="${KESA_CONFORMANCE_FLAKE_LOG:-flake_events.jsonl}"
