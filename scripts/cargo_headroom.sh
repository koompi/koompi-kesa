...
Options:
  --runner <rch|auto|local>   Cargo runner mode (default: KESA_CARGO_RUNNER or rch)
...
Environment:
  KESA_CARGO_BUILD_ROOT         Build root used when CARGO_TARGET_DIR is unset
                              (default: /data/tmp/koompi_code_cli, or
                              /data/tmp/koompi_code_cli_cargo if the former
                              resolves inside this repository)
  KESA_CARGO_AGENT_SUFFIX       Per-agent subdirectory suffix (default: $USER)
  KESA_CARGO_ALLOW_REPO_TARGET  Set to 1 to allow target dirs under the repo root
  KESA_CARGO_ALLOW_LOCAL_FALLBACK
                              Set to 1 to permit heavy local fallback in auto mode
  KESA_CARGO_MAX_LOCAL_PROCESSES
                              Local cargo/rustc process cap for heavy gates
  KESA_CARGO_PROCESS_COUNT      Test/operator override for observed process count
  KESA_CARGO_FORCE_ADMIT        Set to 1 to override local process pressure
  KESA_CARGO_INCLUDE_SCRATCH_CLEANUP
                              Set to 1 to include scratch cleanup pressure on
                              allow decisions too; backoff/degraded decisions
                              include it automatically
  KESA_CARGO_SCRATCH_PLAN_JSON  Test/operator override containing planner JSON
EOF
}
...
RUNNER="${KESA_CARGO_RUNNER:-rch}"
MIN_FREE_MB="${KESA_CARGO_HEADROOM_MIN_FREE_MB:-24576}"
MIN_INODE_FREE_PCT="${KESA_CARGO_HEADROOM_MIN_FREE_INODE_PCT:-5}"
MAX_LOCAL_CARGO_PROCESSES="${KESA_CARGO_MAX_LOCAL_PROCESSES:-2}"
RCH_QUEUE_FORECAST_MAX_AGE_SECS="${KESA_RCH_QUEUE_FORECAST_MAX_AGE_SECS:-120}"
...
BUILD_ROOT="${KESA_CARGO_BUILD_ROOT:-$DEFAULT_BUILD_ROOT}"
TARGET_OVERRIDE=""
TMPDIR_OVERRIDE=""
ADMIT_ONLY=0
DECISION_JSON_PATH=""
ALLOW_LOCAL_FALLBACK="${KESA_CARGO_ALLOW_LOCAL_FALLBACK:-0}"
FORCE_ADMIT="${KESA_CARGO_FORCE_ADMIT:-0}"
...
safe_agent_suffix() {
    printf '%s' "${KESA_CARGO_AGENT_SUFFIX:-${USER:-agent}}" | tr -c 'A-Za-z0-9._-' '_'
}
...
local_process_pressure_json() {
    ...
    if [[ -n "${KESA_CARGO_PROCESS_COUNT:-}" ]]; then
        [[ "$KESA_CARGO_PROCESS_COUNT" =~ ^[0-9]+$ ]] \
            || die "invalid KESA_CARGO_PROCESS_COUNT '$KESA_CARGO_PROCESS_COUNT'"
        count="$KESA_CARGO_PROCESS_COUNT"
        detail="env_override"
...
scratch_cleanup_pressure_json() {
    ...
    if [[ "$decision" == "allow" && "${KESA_CARGO_INCLUDE_SCRATCH_CLEANUP:-0}" != "1" ]]; then
        scratch_cleanup_pressure_not_checked "admission_allowed"
        return 0
    fi

    if [[ -n "${KESA_CARGO_SCRATCH_PLAN_JSON:-}" ]]; then
        summarize_scratch_cleanup_plan "$KESA_CARGO_SCRATCH_PLAN_JSON" "env_override"
        return 0
    fi
...
case "$TARGET_CANDIDATE" in
    "$PROJECT_ROOT"/*)
        if [[ "${KESA_CARGO_ALLOW_REPO_TARGET:-0}" != "1" ]]; then
            die "CARGO_TARGET_DIR is under the repo root ($TARGET_CANDIDATE). Use /data/tmp or set KESA_CARGO_ALLOW_REPO_TARGET=1 explicitly."
        fi
        ;;
esac
...
case "$TMPDIR_CANDIDATE" in
    "$PROJECT_ROOT"/*)
        if [[ "${KESA_CARGO_ALLOW_REPO_TARGET:-0}" != "1" ]]; then
            die "TMPDIR is under the repo root ($TMPDIR_CANDIDATE). Use /data/tmp or set KESA_CARGO_ALLOW_REPO_TARGET=1 explicitly."
        fi
        ;;
esac
...
case "$CARGO_TARGET_DIR" in
    "$PROJECT_ROOT"/*)
        if [[ "${KESA_CARGO_ALLOW_REPO_TARGET:-0}" != "1" ]]; then
            die "CARGO_TARGET_DIR is under the repo root ($CARGO_TARGET_DIR). Use /data/tmp or set KESA_CARGO_ALLOW_REPO_TARGET=1 explicitly."
        fi
        ;;
esac
