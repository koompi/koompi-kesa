...
export KODE_BENCH_RUN_ID="$BENCH_RUN_ID"
export KODE_BENCH_CORRELATION_ID="$BENCH_CORRELATION_ID"
...
for ALLOCATOR_REQUEST in "${ALLOCATOR_SET[@]}"; do
  ...
  for TOOL_CALLS in "${TOOL_CALLS_SET[@]}"; do
    ...
    if [[ "$BENCH_PGO_MODE" == "compare" ]]; then
      ...
      CMD_BASE="KODE_BENCH_BUILD_PROFILE=${BENCH_CARGO_PROFILE} KODE_BENCH_ALLOCATOR=${ALLOCATOR_REQUEST} $BASELINE_BIN --iterations ${ITERATIONS} --tool-calls ${TOOL_CALLS}"
      CMD_PGO="KODE_BENCH_BUILD_PROFILE=${BENCH_CARGO_PROFILE} KODE_BENCH_ALLOCATOR=${ALLOCATOR_REQUEST} $PGO_BIN --iterations ${ITERATIONS} --tool-calls ${TOOL_CALLS}"
      ...
      KODE_BENCH_BUILD_PROFILE="$BENCH_CARGO_PROFILE" KODE_BENCH_ALLOCATOR="$ALLOCATOR_REQUEST" "$PGO_BIN" --iterations "$ITERATIONS" --tool-calls "$TOOL_CALLS" >>"$JSONL_OUT"
    else
      ...
      CMD="KODE_BENCH_BUILD_PROFILE=${BENCH_CARGO_PROFILE} KODE_BENCH_ALLOCATOR=${ALLOCATOR_REQUEST} $ACTIVE_BIN --iterations ${ITERATIONS} --tool-calls ${TOOL_CALLS}"

      hyperfine --warmup "$HYPERFINE_WARMUP" --runs "$HYPERFINE_RUNS" --export-json "$HYPERFINE_OUT" "$CMD"
      KODE_BENCH_BUILD_PROFILE="$BENCH_CARGO_PROFILE" KODE_BENCH_ALLOCATOR="$ALLOCATOR_REQUEST" "$ACTIVE_BIN" --iterations "$ITERATIONS" --tool-calls "$TOOL_CALLS" >>"$JSONL_OUT"
    fi
  done
