...
run_installer() {
  ...
  local run_cwd="${KESA_INSTALLER_TEST_CWD:-$PWD}"
...
test_installer_retain_temp_mode_preserves_owned_scratch() {
  ...
  KESA_INSTALLER_LOCK_DIR="$lock_dir" \
  TMPDIR="$retained_tmp" \
  run_installer "$dir" \
    --yes --no-gum --offline \
    --version v9.9.9 \
...
test_lock_override_rejects_ambiguous_lexical_paths() {
  ...
  for unsafe_lock in \
    ...
    KESA_INSTALLER_LOCK_DIR="$unsafe_lock" \
    run_installer "$dir" --yes --no-gum

    assert_exit_code "$dir" 1
    assert_output_contains "$dir" "KESA_INSTALLER_LOCK_DIR is unsafe"
  done
}
...
test_wsl_detection_warning_is_emitted() {
  ...
  KESA_INSTALLER_TEST_FORCE_WSL=1 \
  run_installer "$dir" \
    --yes --no-gum --offline \
    --version v9.9.9 \
    --dest "${dir}/dest" \
...
SKILL
  ...
  KESA_INSTALLER_TEST_CWD="${dir}/shadow" run_installer "$dir" \
    --yes --no-gum --offline \
    --version v9.9.9 \
    --dest "${dir}/dest" \
    --artifact-url "${artifact_url}" \
...
test_completions_help_conclusive_no_command_skips_fast() {
  ...
  KESA_INSTALLER_COMPLETION_PROBE_TIMEOUT=1 \
...
test_completions_generation_timeout_is_non_fatal() {
  ...
  KESA_INSTALLER_COMPLETION_CMD_TIMEOUT=1 \
  STUB_COMPLETION_SLEEP_SECS=3 \
  run_installer "$dir" \
    --yes --no-gum --offline \
    --version v9.9.9 \
