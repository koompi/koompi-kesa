...
//! - Full 1-hour: `KESA_MEM_STRESS_DURATION_SECS=3600`
//!
//! ## Environment Variables
//!
//! | Variable                       | Default | Description                           |
//! |--------------------------------|---------|---------------------------------------|
//! | `KESA_MEM_STRESS_DURATION_SECS`  | 60      | Total run duration                    |
//! | `KESA_MEM_STRESS_EVENTS_PER_SEC` | 17      | Event dispatch rate (~1000/min)       |
//! | `KESA_MEM_STRESS_RSS_INTERVAL`   | 5       | Seconds between RSS samples           |
//! | `KESA_MEM_STRESS_MAX_EXTENSIONS` | 20      | Max extensions to load simultaneously |
//!
//! ## Output
//!
...
fn duration_secs() -> u64 {
    std::env::var("KESA_MEM_STRESS_DURATION_SECS")
        .ok()
...
fn events_per_sec() -> u64 {
    std::env::var("KESA_MEM_STRESS_EVENTS_PER_SEC")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(17)
}
...
fn rss_interval_secs() -> u64 {
    std::env::var("KESA_MEM_STRESS_RSS_INTERVAL")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(5)
}
...
fn max_extensions() -> usize {
    std::env::var("KESA_MEM_STRESS_MAX_EXTENSIONS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(20)
}
...
fn ext_memory_stress_real_extensions() {
    if std::env::var("KESA_MEM_STRESS_REAL").is_err() {
        eprintln!(
            "[mem-stress] skipping real extension stress test \
             (set KESA_MEM_STRESS_REAL=1 to enable)"
        );
        return;
    }
