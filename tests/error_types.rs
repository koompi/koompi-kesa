...
fn test_session_not_found_variant() {
    let err = Error::SessionNotFound {
        path: "/home/user/.kesa/sessions/test.jsonl".to_string(),
    };

    assert_display_contains(&err, "Session not found");
    assert_display_contains(&err, "/home/user/.kesa/sessions/test.jsonl");
}
...
fn test_session_not_found_preserves_path() {
    let paths = [
        "/home/user/.kesa/sessions/test.jsonl",
        "./relative/path.jsonl",
        "~/.kesa/sessions/session.jsonl",
    ];
...
fn hints_config_missing_file() {
    let err = Error::config("config file not found at /home/user/.kesa/settings");
    let h = err.hints();
...
fn hints_session_not_found() {
    let err = Error::SessionNotFound {
        path: "/home/user/.kesa/sessions/abc.jsonl".to_string(),
    };
    let h = err.hints();
