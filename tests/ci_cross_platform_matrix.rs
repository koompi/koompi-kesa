...
//!   `KESA_GENERATE_CROSS_PLATFORM_MATRIX=1 cargo test --test ci_cross_platform_matrix cross_platform_matrix -- --exact --nocapture`

use serde_json::Value;
use std::path::{Path, PathBuf};

const GENERATE_CROSS_PLATFORM_MATRIX_ENV: &str = "KESA_GENERATE_CROSS_PLATFORM_MATRIX";
...
fn merge_policy() -> MergePolicy {
    MergePolicy {
        description: "Cross-platform merge policy for koompi_code_cli CI".to_string(),
