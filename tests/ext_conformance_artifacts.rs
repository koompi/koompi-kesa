use kesa::extensions::CompatibilityScanner;
...
fn test_ext_conformance_pinned_sample_compat_ledger_snapshot() {
    ...
    let mut ledgers: BTreeMap<String, kesa::extensions::CompatLedger> = BTreeMap::new();
    for id in ids {
        let artifact_dir = repo_root.join("tests/ext_conformance/artifacts").join(&id);
        assert!(
            artifact_dir.is_dir(),
...
fn test_snapshot_protocol_digest_matches_local_implementation() {
    use kesa::conformance::snapshot::digest_artifact_dir as lib_digest;
