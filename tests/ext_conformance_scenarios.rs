...
use kesa::conformance::normalization::{is_path_key, path_suffix_match};
use kesa::extensions::{
...
use kesa::extensions_js::{HostcallKind, HostcallRequest, PiJsRuntimeConfig};
use kesa::resources::{
...
fn deterministic_settings() -> DeterministicSettings {
    let random_env = std::env::var("KESA_DETERMINISTIC_RANDOM")
        .ok()
        .filter(|val| !val.trim().is_empty());
    let seed_env = std::env::var("KESA_DETERMINISTIC_RANDOM_SEED")
        .ok()
        .filter(|val| !val.trim().is_empty());
    ...
    DeterministicSettings {
        time_ms: env_or_default("KESA_DETERMINISTIC_TIME_MS", DEFAULT_DETERMINISTIC_TIME_MS),
        time_step_ms: env_or_default(
            "KESA_DETERMINISTIC_TIME_STEP_MS",
            DEFAULT_DETERMINISTIC_TIME_STEP_MS,
        ),
        random_seed: env_or_default(
            "KESA_DETERMINISTIC_RANDOM_SEED",
            DEFAULT_DETERMINISTIC_RANDOM_SEED,
        ),
        random_value,
        cwd: env_or_default("KESA_DETERMINISTIC_CWD", DEFAULT_DETERMINISTIC_CWD),
        home: env_or_default("KESA_DETERMINISTIC_HOME", DEFAULT_DETERMINISTIC_HOME),
    }
}
...
fn deterministic_settings_for(extension_path: &Path) -> DeterministicSettings {
    ...
    if std::env::var("KESA_DETERMINISTIC_CWD").is_err() {
        settings.cwd = Path::new(DEFAULT_DETERMINISTIC_CWD)
            .join(&key)
            .display()
            .to_string();
    }
    if std::env::var("KESA_DETERMINISTIC_HOME").is_err() {
...
fn parse_scenario_shard(
    ...
    match (index_raw, total_raw) {
        ...
        (Some(_), None) | (None, Some(_)) => Err(
            "both KESA_SCENARIO_SHARD_INDEX and KESA_SCENARIO_SHARD_TOTAL are required".to_string(),
        ),
        (Some(index_raw), Some(total_raw)) => {
            let index = index_raw
                .parse::<usize>()
                .map_err(|err| format!("invalid KESA_SCENARIO_SHARD_INDEX='{index_raw}': {err}"))?;
            let total = total_raw
                .parse::<usize>()
                .map_err(|err| format!("invalid KESA_SCENARIO_SHARD_TOTAL='{total_raw}': {err}"))?;

            if total == 0 {
                return Err("KESA_SCENARIO_SHARD_TOTAL must be > 0".to_string());
            }
            if index >= total {
                return Err(format!(
                    "KESA_SCENARIO_SHARD_INDEX must be < KESA_SCENARIO_SHARD_TOTAL ({index} >= {total})"
                ));
            }
...
fn scenario_shard_from_env() -> Option<ScenarioShard> {
    let index = std::env::var("KESA_SCENARIO_SHARD_INDEX").ok();
    let total = std::env::var("KESA_SCENARIO_SHARD_TOTAL").ok();
    let name = std::env::var("KESA_SCENARIO_SHARD_NAME").ok();
...
fn load_extension(extension_path: &Path) -> Result<LoadedExtension, String> {
    ...
    env.insert(
        "KESA_DETERMINISTIC_TIME_MS".to_string(),
    ...
    env.insert("KESA_EXT_COMPAT_SCAN".to_string(), "0".to_string());
    if let Some(random_value) = settings.random_value {
        env.insert("KESA_DETERMINISTIC_RANDOM".to_string(), random_value);
    } else {
        env.insert(
            "KESA_DETERMINISTIC_RANDOM_SEED".to_string(),
...
impl ExtensionSession for ConformanceSession {
    ...
    async fn set_name(&self, name: String) -> kesa::error::Result<()> {
        *self.name.lock().unwrap() = Some(name);
        Ok(())
    }

    async fn append_message(&self, message: SessionMessage) -> kesa::error::Result<()> {
    ...
    async fn set_model(&self, provider: String, model_id: String) -> kesa::error::Result<()> {
    ...
    async fn set_thinking_level(&self, level: String) -> kesa::error::Result<()> {
    ...
    async fn set_label(&self, target_id: String, label: Option<String>) -> kesa::error::Result<()> {
...
fn parse_scenario_shard_rejects_partial_values() {
    ...
    assert!(err.contains("both KESA_SCENARIO_SHARD_INDEX and KESA_SCENARIO_SHARD_TOTAL"));
}
...
fn scenario_conformance_suite() {
    ...
    let filter = std::env::var("KESA_SCENARIO_FILTER").ok();
...
impl ExtensionHostActions for PiAiProviderBridgeHostActions {
    async fn send_message(&self, _message: ExtensionSendMessage) -> kesa::error::Result<()> {
        Ok(())
    }
    ...
    async fn list_ai_models(&self) -> kesa::error::Result<Value> {
        Ok(serde_json::json!([
            {
                "id": "mock-model",
                "provider": "mock-provider",
...
fn ts_oracle_timeout() -> Duration {
    std::env::var("KESA_TS_ORACLE_TIMEOUT_SECS")
        .ok()
...
fn run_ts_scenario(extension_path: &Path, scenario: &Scenario) -> Result<Value, String> {
    ...
    cmd.arg("run")
        ...
        .env("KESA_DETERMINISTIC_TIME_MS", &settings.time_ms)
        .env("KESA_DETERMINISTIC_TIME_STEP_MS", &settings.time_step_ms)
        .env("KESA_DETERMINISTIC_CWD", &settings.cwd)
        .env("KESA_DETERMINISTIC_HOME", &settings.home);
    if let Some(random_value) = settings.random_value.as_deref() {
        cmd.env("KESA_DETERMINISTIC_RANDOM", random_value);
    } else {
        cmd.env("KESA_DETERMINISTIC_RANDOM_SEED", &settings.random_seed);
    }
