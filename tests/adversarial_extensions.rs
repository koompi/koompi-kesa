...
use kesa::extensions_js::{PiJsRuntimeConfig, PiJsRuntimeLimits};
...
// Verify that KESA_API_KEY is still blocked by suffix match.
// ═══════════════════════════════════════════════════════════════════════════════

#[test]
fn t6_pi_api_key_suffix_still_blocked() {
    // KESA_API_KEY matches *_API_KEY suffix — should be blocked even though
    // PI_* prefix is allowed. Blocklist is checked before the allow-list.
    let result = eval_adversarial(
        r#"
export default function activate(pi) {
  pi.on("agent_start", () => {
    const key = process.env.KESA_API_KEY;
    return { result: key === undefined ? "BLOCKED" : "LEAKED:" + key };
  });
}
"#,
    ...
    assert_eq!(
        result, "BLOCKED",
        "KESA_API_KEY should be blocked by *_API_KEY suffix despite PI_* prefix allow"
    );
}
...
export default function activate(pi) {
  pi.on("agent_start", () => {
    // KESA_TEST_MODE is a legitimate PI_* var
    const val = process.env.KESA_TEST_MODE;
...
"#,
    ...
    assert!(
        ...
        "KESA_TEST_MODE (PI_* prefix, no blocklist match) should be allowed, got: {result}"
    );
}
