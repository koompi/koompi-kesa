...
const KESA_MONO_ROOT_RELATIVE: &str = "legacy_pi_mono_code/pi-mono";
const KESA_MONO_TSX_RELATIVE: &str = "node_modules/tsx/dist/cli.mjs";
const KESA_MONO_CLI_RELATIVE: &str = "packages/coding-agent/src/cli.ts";
...
impl RunnerPaths {
    fn discover() -> anyhow::Result<Self> {
        let rust_pi = PathBuf::from(env!("CARGO_BIN_EXE_kesa"));
        if !rust_pi.is_file() {
            anyhow::bail!("missing Rust Pi test binary: {}", rust_pi.display());
        }

        let repo = Path::new(env!("CARGO_MANIFEST_DIR"));
        let pi_mono_root = repo.join(KESA_MONO_ROOT_RELATIVE);
        if !pi_mono_root.is_dir() {
            anyhow::bail!("missing pinned pi-mono root: {}", pi_mono_root.display());
        }

        let pi_mono_tsx = pi_mono_root.join(KESA_MONO_TSX_RELATIVE);
        ...
        let pi_mono_cli = pi_mono_root.join(KESA_MONO_CLI_RELATIVE);
...
fn run_rust_rpc_sequence(
    ...
    let mut child = Command::new(&paths.rust_pi)
        ...
        .env("KESA_SESSIONS_DIR", &sessions_dir)
