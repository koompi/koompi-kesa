...
//! `KESA_EXT_RANDOM_N`. Results are written as JSONL logs and a summary manifest.
//!
//! # Environment Variables
//! - `KESA_EXT_RANDOM_SEED` - u64 seed for deterministic selection (default: 42)
//! - `KESA_EXT_RANDOM_N` - sample size (default: 1; use 20 for the legacy batch)
//! - `KESA_EXT_RANDOM_FILTER` - optional `tier:1-3` or `source:community` filter
//! - `KESA_EXT_RANDOM_IDS` - optional comma-separated explicit ID list (bypasses selector)
//! - `KESA_EXT_RANDOM_OUTPUT_DIR` - optional output directory (default: `$TMPDIR/koompi_code_cli/...`)
//!
//! # Smoke Run
//! ```sh
...
//! # Wider Opt-In Batch
//! ```sh
//! KESA_EXT_RANDOM_SEED=42 KESA_EXT_RANDOM_N=20 \
//!   rch exec -- cargo test --features ext-conformance --test ext_random_trials random_trials_batch -- --nocapture
//! ```
...
const RANDOM_OUTPUT_DIR_ENV: &str = "KESA_EXT_RANDOM_OUTPUT_DIR";
...
fn trial_config_from_env() -> TrialConfig {
    TrialConfig {
        seed: std::env::var("KESA_EXT_RANDOM_SEED")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(DEFAULT_RANDOM_SEED),
        sample_size: std::env::var("KESA_EXT_RANDOM_N")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(DEFAULT_RANDOM_SAMPLE_SIZE),
        filter_str: std::env::var("KESA_EXT_RANDOM_FILTER").unwrap_or_default(),
        explicit_ids: std::env::var("KESA_EXT_RANDOM_IDS").ok(),
    }
}
...
/// `KESA_EXT_RANDOM_N` for a wider opt-in batch.
#[test]
