...
fn config_repair_mode_defaults_to_auto_safe() {
    ...
    assert_eq!(
        config.repair_mode,
        kesa::extensions_js::RepairMode::AutoSafe
    );
...
fn config_repair_mode_off_disables_repair() {
    let config = PiJsRuntimeConfig {
        repair_mode: kesa::extensions_js::RepairMode::Off,
        ..Default::default()
    };
    assert!(!config.auto_repair_enabled());
}
...
fn config_repair_mode_suggest_does_not_apply() {
    let config = PiJsRuntimeConfig {
        repair_mode: kesa::extensions_js::RepairMode::Suggest,
        ..Default::default()
    };
    assert!(!config.auto_repair_enabled());
...
fn config_repair_mode_auto_strict_enables_aggressive() {
    let config = PiJsRuntimeConfig {
        repair_mode: kesa::extensions_js::RepairMode::AutoStrict,
        ..Default::default()
    };
    assert!(config.auto_repair_enabled());
...
use kesa::extensions_js::{applicable_rules, rule_by_id};
...
fn monorepo_escape_stub_loads_with_autostrict() {
    use kesa::extensions::{ExtensionEventName, JsExtensionLoadSpec};
...
fn monorepo_escape_not_applied_in_autosafe_mode() {
    use kesa::extensions::JsExtensionLoadSpec;
