...
use kesa::model::{Message, ThinkingLevel, UserContent};
use kesa::models::CompatConfig;
use kesa::provider::{Context, Provider, StreamOptions, ThinkingBudgets, ToolDef};
...
fn minimal_context() -> Context<'static> {
    Context {
        ...
        messages: vec![Message::User(kesa::model::UserMessage {
...
fn oai_compat_entry(provider: &str, base_url: &str) -> kesa::models::ModelEntry {
    use kesa::provider::{InputType, Model, ModelCost};
...
fn factory_routes_gitlab_native_provider() {
    ...
    let entry = kesa::models::ModelEntry {
        model: Model {
