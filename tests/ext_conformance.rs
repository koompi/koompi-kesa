...
//! [`kesa::conformance::normalization`].  This test file delegates to that
...
fn parse_and_normalize_jsonl(
    ...
    for (idx, line) in input.lines().enumerate() {
        ...
        if std::env::var_os("KESA_TEST_MODE").is_some() {
            trace!(
...
fn normalizes_dynamic_fields_paths_and_ansi() {
    ...
    assert!(
        msg.contains("<KESA_MONO_ROOT>/file.txt") || msg.contains("<KESA_MONO_ROOT>\\file.txt"),
    );
    ...
    assert!(
        path.contains("<KESA_MONO_ROOT>/dir/sub/file.rs")
            || path.contains("<KESA_MONO_ROOT>\\dir\\sub\\file.rs"),
    );
...
fn trace_viewer_renders_pretty_and_exports_jsonl() {
    ...
    let binary_path = PathBuf::from(env!("CARGO_BIN_EXE_kesa_legacy_capture"));
...
fn regression_dynamic_resources_full_normalization_pipeline() {
    ...
    // Paths in data should be rewritten to use KESA_MONO_ROOT placeholder
