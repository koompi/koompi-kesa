...
mod read_tool {
    ...
    fn test_read_existing_file() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::ReadTool::new(temp_dir.path());
    ...
    fn test_read_first_line_exceeds_limit_sets_truncation_details() {
        asupersync::test_utils::run_test(|| async {
            ...
            let long_line = "a".repeat(kesa::tools::DEFAULT_MAX_BYTES + 128);
            ...
            let expected_limit = format!(
                "exceeds {} limit",
                format_size(kesa::tools::DEFAULT_MAX_BYTES)
            );
            assert!(
    ...
    fn test_read_truncation_sets_details_and_hint() {
        asupersync::test_utils::run_test(|| async {
            ...
            let total_lines = kesa::tools::DEFAULT_MAX_LINES + 5;
            ...
            let expected_offset = format!("Use offset={}", kesa::tools::DEFAULT_MAX_LINES + 1);
            assert!(
    ...
    fn test_read_blocked_images_returns_error() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::ReadTool::with_settings(harness.temp_dir(), true, true);
            let input = serde_json::json!({
                "path": path.to_string_lossy()
            });
...
mod file_arguments {
    ...
    fn test_at_file_permission_owner_class_does_not_borrow_other_read_bit() {
        ...
        let err = kesa::tools::process_file_arguments(&file_args, harness.temp_dir(), true)
...
mod write_tool {
    ...
    fn test_write_new_file() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::WriteTool::new(temp_dir.path());
            let input = serde_json::json!({
                "path": test_file.to_string_lossy(),
                "content": content
            });
...
mod edit_tool {
    ...
    fn test_edit_replace_text() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::EditTool::new(temp_dir.path());
            let input = serde_json::json!({
                "path": test_file.to_string_lossy(),
                "oldText": "World",
                "newText": "Rust"
...
mod bash_tool {
    ...
    fn test_bash_simple_command() {
        asupersync::test_utils::run_test(|| async {
            let temp_dir = tempfile::tempdir().unwrap();
            let tool = kesa::tools::BashTool::new(temp_dir.path());
            let input = serde_json::json!({
...
mod grep_tool {
    ...
    fn test_grep_basic_pattern() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::GrepTool::new(temp_dir.path());
            let input = serde_json::json!({
                "pattern": "hello"
            });
    ...
    fn test_grep_hashline_tags_match_read_output() {
        asupersync::test_utils::run_test(|| async {
            ...
            let read_tool = kesa::tools::ReadTool::new(temp_dir.path());
            ...
            let grep_tool = kesa::tools::GrepTool::new(temp_dir.path());
...
mod find_tool {
    ...
    fn test_find_glob_pattern() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::FindTool::new(temp_dir.path());
            let input = serde_json::json!({
                "pattern": "*.txt"
            });
...
mod ls_tool {
    ...
    fn test_ls_directory() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::LsTool::new(temp_dir.path());
            let input = serde_json::json!({});
...
fn get_text_content(content: &[kesa::model::ContentBlock]) -> String {
    content
        .iter()
        .filter_map(|block| {
            if let kesa::model::ContentBlock::Text(text) = block {
...
fn tool_command_transcript(
    ...
    result: &kesa::PiResult<kesa::tools::ToolOutput>,
) -> serde_json::Value {
    match result {
...
async fn execute_tool_with_diagnostics<T: Tool + ?Sized>(
    ...
    tool_call_id: &str,
    input: serde_json::Value,
) -> kesa::PiResult<kesa::tools::ToolOutput> {
...
mod e2e_read {
    ...
    fn e2e_read_truncation_details_captured() {
        asupersync::test_utils::run_test(|| async {
            ...
            let total_lines = kesa::tools::DEFAULT_MAX_LINES + 10;
...
fn e2e_all_tools_roundtrip() {
    asupersync::test_utils::run_test(|| async {
        ...
        // Write a file
        let write_tool = kesa::tools::WriteTool::new(harness.temp_dir());
        ...
        let read_tool = kesa::tools::ReadTool::new(harness.temp_dir());
        ...
        let edit_tool = kesa::tools::EditTool::new(harness.temp_dir());
        ...
        // Ls the directory
        harness.section("Ls");
        let ls_tool = kesa::tools::LsTool::new(harness.temp_dir());
        ...
        let bash_tool = kesa::tools::BashTool::new(harness.temp_dir());
        ...
        if binary_available("rg") {
            ...
            let grep_tool = kesa::tools::GrepTool::new(harness.temp_dir());
        ...
        if binary_available("fd") {
            ...
            let find_tool = kesa::tools::FindTool::new(harness.temp_dir());
...
mod security_path_traversal {
    ...
    fn read_parent_dir_traversal() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::ReadTool::new(&child_dir);
            let input = serde_json::json!({
                "path": "../outside.txt"
            });
            let err = tool
    ...
    fn write_parent_dir_traversal() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::WriteTool::new(&child_dir);
    ...
    fn edit_parent_dir_traversal() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::EditTool::new(&child_dir);
    ...
    fn read_absolute_path_outside_cwd() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::ReadTool::new(cwd.path());
            let input = serde_json::json!({
    ...
    fn write_absolute_path_outside_cwd() {
        asupersync::test_utils::run_test(|| async {
            ...
            let cwd = tempfile::tempdir().unwrap();
            let tool = kesa::tools::WriteTool::new(cwd.path());
            let input = serde_json::json!({
                "path": outside_file.to_string_lossy(),
                "content": "NOPE"
            });
    ...
    fn edit_absolute_path_outside_cwd() {
        asupersync::test_utils::run_test(|| async {
            ...
            let cwd = tempfile::tempdir().unwrap();
            let tool = kesa::tools::EditTool::new(cwd.path());
            let input = serde_json::json!({
                "path": outside_file.to_string_lossy(),
                "oldText": "ORIGINAL",
                "newText": "MODIFIED"
...
mod security_command_injection {
    ...
    fn bash_stdin_is_null() {
        asupersync::test_utils::run_test(|| async {
            let cwd = tempfile::tempdir().unwrap();
            let tool = kesa::tools::BashTool::new(cwd.path());
            let input = serde_json::json!({
...
mod hashline_edit_tool {
    ...
    fn test_basic_replace() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::HashlineEditTool::new(temp_dir.path());
            let input = serde_json::json!({
                "path": test_file.to_string_lossy(),
                "edits": [{
                    "op": "replace",
    ...
    fn test_parent_without_read_is_rejected_before_hashline_atomic_mutation() {
        asupersync::test_utils::run_test(|| async {
            ...
            let tool = kesa::tools::HashlineEditTool::new(harness.temp_dir());
            let err = tool
                .execute(
                    "test-id",
                    serde_json::json!({
