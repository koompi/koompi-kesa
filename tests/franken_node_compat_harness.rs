...
//! Set `KESA_GENERATE_FRANKEN_NODE_COMPATIBILITY_MATRIX=1` to regenerate the
...
fn verify_or_generate_compatibility_matrix(
    ...
    let generate = matches!(
        std::env::var("KESA_GENERATE_FRANKEN_NODE_COMPATIBILITY_MATRIX").as_deref(),
        Ok("1")
    );
    if generate {
    ...
    assert_eq!(
        ...
        "committed FrankenNode compatibility matrix is stale; regenerate explicitly with \
         KESA_GENERATE_FRANKEN_NODE_COMPATIBILITY_MATRIX=1 cargo test \
