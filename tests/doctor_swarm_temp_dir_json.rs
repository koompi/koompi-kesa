...
fn run_doctor_json(env_overrides: &[(&str, Option<&str>)]) -> TestResult<Value> {
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_kesa")); // ubs:ignore false positive: Cargo provides the compiled test binary path.
    command
        ...
        .env_remove("KESA_VALIDATION_BROKER_STORE")
        .env_remove("KESA_DOCTOR_LOGICAL_CPU_CORES")
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
fn doctor_swarm_validation_broker_json_reports_missing_configured_store() -> TestResult {
    ...
    let report = run_doctor_json(&[
        ...
        ("TMPDIR", None),
        ("KESA_VALIDATION_BROKER_STORE", Some(missing_store.as_str())),
    ])?;
    let finding = finding_by_schema(&report, SWARM_VALIDATION_BROKER_SCHEMA)?;
    require_eq(field_str(finding, "severity")?, "warn", "severity")?;
    require(
...
fn doctor_swarm_validation_broker_json_reports_stale_slot_posture() -> TestResult {
    ...
    let report = run_doctor_json(&[
        ("CARGO_TARGET_DIR", None),
        ("TMPDIR", None),
        ("KESA_VALIDATION_BROKER_STORE", Some(store_path.as_str())),
    ])?;
    let finding = finding_by_schema(&report, SWARM_VALIDATION_BROKER_SCHEMA)?;
    require_eq(field_str(finding, "severity")?, "warn", "severity")?;
    require(
...
fn doctor_swarm_resource_preflight_json_reports_constrained_profile() -> TestResult {
    ...
    let report = run_doctor_json(&[
        ...
        ("KESA_DOCTOR_CGROUP_CPU_MAX_PATH", Some(cpu_max.as_str())),
        ("KESA_DOCTOR_CPUSET_CPUS_PATH", Some(cpuset.as_str())),
        ("KESA_DOCTOR_NUMA_ONLINE_PATH", Some(numa.as_str())),
        ("KESA_DOCTOR_CGROUP_MEMORY_MAX_PATH", Some(memory.as_str())),
        ("KESA_DOCTOR_MEMINFO_PATH", Some(meminfo.as_str())),
        ("KESA_DOCTOR_LOGICAL_CPU_CORES", Some("4")),
        ("KESA_DOCTOR_LOCAL_BUILD_PROCESS_COUNT", Some("0")),
    ])?;
...
fn doctor_swarm_resource_preflight_json_reports_high_capacity_profile() -> TestResult {
    ...
    let report = run_doctor_json(&[
        ...
        ("KESA_DOCTOR_CGROUP_MEMORY_MAX_PATH", Some(memory.as_str())),
        ("KESA_DOCTOR_MEMINFO_PATH", Some(meminfo.as_str())),
        ("KESA_DOCTOR_LOGICAL_CPU_CORES", Some("64")),
        ("KESA_DOCTOR_LOCAL_BUILD_PROCESS_COUNT", Some("0")),
    ])?;
    let finding = finding_by_schema(&report, SWARM_RESOURCE_PREFLIGHT_SCHEMA)?;
    let data = field(finding, "data")?;
...
fn doctor_swarm_resource_preflight_json_reports_unknown_topology_lane_plan() -> TestResult {
    ...
    let report = run_doctor_json(&[
        ...
        ("KESA_DOCTOR_CGROUP_CPU_MAX_PATH", Some(cpu_max.as_str())),
        (
            "KESA_DOCTOR_CPUSET_CPUS_PATH",
            Some(missing_cpuset.as_str()),
        ),
        ("KESA_DOCTOR_NUMA_ONLINE_PATH", Some(missing_numa.as_str())),
        ("KESA_DOCTOR_CGROUP_MEMORY_MAX_PATH", Some(memory.as_str())),
        ("KESA_DOCTOR_MEMINFO_PATH", Some(meminfo.as_str())),
        ("KESA_DOCTOR_LOCAL_BUILD_PROCESS_COUNT", Some("0")),
    ])?;
