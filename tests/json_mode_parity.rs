...
use kesa::agent::{AgentEvent, TURN_LATENCY_BREAKDOWN_SCHEMA_V1, TurnLatencyBreakdown};
use kesa::extensions::{ExtensionEventName, ExtensionUiRequest, extension_event_from_agent};
use kesa::model::{
...
fn test_user_message() -> Message {
    Message::User(kesa::model::UserMessage {
        content: kesa::model::UserContent::Text("test prompt".to_string()),
        timestamp: 1_700_000_000,
    })
}
...
fn json_parity_ui_response_value_variant() {
    ...
    let resp = kesa::extensions::ExtensionUiResponse {
        id: "sel-1".to_string(),
...
fn json_parity_ui_response_confirmed_variant() {
    ...
    let confirmed = kesa::extensions::ExtensionUiResponse {
        id: "cfm-1".to_string(),
    ...
    let denied = kesa::extensions::ExtensionUiResponse {
        id: "cfm-2".to_string(),
...
fn json_parity_ui_response_text_value() {
    ...
    let input_resp = kesa::extensions::ExtensionUiResponse {
        id: "inp-1".to_string(),
    ...
    let editor_resp = kesa::extensions::ExtensionUiResponse {
        id: "edt-1".to_string(),
...
fn json_parity_agent_end_message_ordering() {
    ...
    let msgs: Vec<Message> = (0..5)
        .map(|i| {
            Message::User(kesa::model::UserMessage {
                content: kesa::model::UserContent::Text(format!("message {i}")),
