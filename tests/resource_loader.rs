...
fn load_skills_defaults_and_collision_diagnostics() {
    ...
    let project_alpha_dir = cwd.join(".kesa").join("skills").join("alpha");
    let project_beta_dir = cwd.join(".kesa").join("skills").join("beta");
...
fn prompt_template_description_and_collision_diagnostics() {
    ...
    let project_plan = write_prompt(
        &harness,
        &cwd.join(".kesa").join("prompts").join("plan.md"),
...
fn themes_load_ini_and_dedupe_collisions() {
    ...
    let project_dark = write_theme_ini(
        &harness,
        &cwd.join(".kesa").join("themes").join("dark.ini"),
        "brand.accent = bold #38bdf8",
    );
...
fn resolved_paths_feed_skill_and_prompt_loaders_with_collision_diagnostics() {
    ...
    let project_base_dir = cwd.join(".kesa");
    ...
    assert!(
        alpha
            .file_path
            .ends_with(Path::new("project/.kesa/skills/alpha/SKILL.md")),
    ...
    assert!(
        collision
            .file_path
            .ends_with(Path::new("project/.kesa/prompts/collision.md")),
