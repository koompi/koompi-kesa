...
//!   KESA_GENERATE_SECURITY_COMPAT_DASHBOARD=1 cargo test --test security_conformance_benign generate_compat_dashboard_artifact -- --exact --nocapture
//!   KESA_GENERATE_SECURITY_COMPAT_EVENTS=1 cargo test --test security_conformance_benign emit_compat_events_jsonl -- --exact --nocapture
//!
...
const GENERATE_SECURITY_COMPAT_DASHBOARD_ENV: &str = "KESA_GENERATE_SECURITY_COMPAT_DASHBOARD";
const GENERATE_SECURITY_COMPAT_EVENTS_ENV: &str = "KESA_GENERATE_SECURITY_COMPAT_EVENTS";
...
fn benign_workflow_produces_no_security_alerts() {
    ...
    let alerts = kesa::extensions::query_security_alerts(&manager, &filter);
    assert!(
        alerts.is_empty(),
