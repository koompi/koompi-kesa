...
//! ## Modes
//!
//! - `KESA_BENCH_MODE=pr`      — diverse subset (10 extensions, 10 iterations) for PR CI
//! - `KESA_BENCH_MODE=nightly`  — full corpus (all safe extensions, 50 iterations)
//! - `KESA_BENCH_MODE=custom`   — use `KESA_BENCH_MAX` and `KESA_BENCH_ITERATIONS`
//!
...
//!
//! Per-extension timeout: `KESA_BENCH_TIMEOUT_SECS` (default 30s) aborts slow extensions.
//!
//! ## Scenarios
//!
...
fn bench_mode() -> BenchMode {
    match std::env::var("KESA_BENCH_MODE")
...
fn iterations() -> usize {
    std::env::var("KESA_BENCH_ITERATIONS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or_else(|| match bench_mode() {
            BenchMode::Pr => 10,
...
fn event_dispatch_count() -> usize {
    std::env::var("KESA_BENCH_EVENT_COUNT")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or_else(|| match bench_mode() {
            BenchMode::Pr => 50,
...
fn per_extension_timeout() -> Duration {
    let secs: u64 = std::env::var("KESA_BENCH_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(30);
