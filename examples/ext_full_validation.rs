...
fn run_commands(project_root: &Path, args: &Args, stages: &mut Vec<StageResult>) -> Result<()> {
    ...
    for shard_index in 0..args.shards {
        let env = vec![
            ("KESA_SHARD_INDEX".to_string(), shard_index.to_string()),
            ("KESA_SHARD_TOTAL".to_string(), args.shards.to_string()),
            (
                "KESA_SHARD_PARALLELISM".to_string(),
...
fn classify_item(
    item: &kesa::extension_popularity::CandidateItem,
...
fn derive_candidate_id_variants(item: &kesa::extension_popularity::CandidateItem) -> Vec<String> {
    let mut out = Vec::new();
