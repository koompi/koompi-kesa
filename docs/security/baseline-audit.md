...
This audit documents the concrete security controls in `koompi_code_cli`'s extension
...
- `PI_*` vars unconditionally allowed (line 125-127) — but this is checked
  *after* the blocklist, so `KESA_API_KEY` would still be blocked by suffix match
