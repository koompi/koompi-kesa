...
KESA_CARGO_AGENT_SUFFIX="$USER" ./scripts/cargo_headroom.sh --runner rch clippy --all-targets -- -D warnings
```
...
`--allow-local-fallback` / `KESA_CARGO_ALLOW_LOCAL_FALLBACK=1`. If `rch` is
...
KESA_OFFICIAL_MAX=5 cargo test --test ext_conformance_diff --features ext-conformance -- --nocapture
...
rch exec -- env KESA_NPM_FILTER=aliou-pi-extension-dev KESA_NPM_MAX=1 \
...
|----------|---------|---------|
| `KESA_OFFICIAL_MAX` | (all) | Limit official extensions tested |
| `KESA_NPM_FILTER` | (none) | Filter npm-registry extensions by `dir/entry` substring |
| `KESA_NPM_MAX` | 5 | Limit the ignored npm-registry differential lane to a deterministic bounded sample |
| `KESA_TS_ORACLE_TIMEOUT_SECS` | 30 | TS oracle process timeout |
| `KESA_DETERMINISTIC_TIME_MS` | 1700000000000 | Fixed wall-clock for determinism |
| `KESA_DETERMINISTIC_RANDOM_SEED` | 1337 | Fixed random seed |
