...
[[ -z "${KESA_CRATES_IO_RELEASE_TOKEN:-}" ]]
...
test -z "${KESA_CRATES_IO_RELEASE_TOKEN:-}"
...
3. Generate source-bound conformance evidence explicitly. Do not copy forward
   ...
   release_build_env CI_RUN_ID="$CI_RUN_ID" CI_CORRELATION_ID="$CI_CORRELATION_ID" \
     KESA_GENERATE_CONFORMANCE_REPORT=1 \
...
8. On the clean publisher checkout at the exact tagged commit, materialize and
   ...
   def invoke(label, request):
       ...
       env = {
           **os.environ,
           "KESA_CRATES_IO_RELEASE_TOKEN": "self-test-token",
           "KESA_EXPECTED_CRATE_NAME": "koompi_code_cli",
           "KESA_EXPECTED_CRATE_VERSION": os.environ["PACKAGE_VERSION"],
           "KESA_EXPECTED_CRATE_SHA256": os.environ["CRATE_SHA256"],
           "KESA_CREDENTIAL_RECEIPT": str(receipt),
       }
       process = subprocess.run(
   ...
   publish_exact_crate_with_scoped_token() {
     ...
     builtin printf '%s\n' "$controller_token" |
       publisher_env \
         KESA_EXPECTED_CRATE_NAME=koompi_code_cli \
         KESA_EXPECTED_CRATE_VERSION="$RELEASE_VERSION" \
         KESA_EXPECTED_CRATE_SHA256="$expected_crate_sha256" \
         KESA_CREDENTIAL_RECEIPT="$credential_receipt" \
         "$release_bash_path" --noprofile --norc -c '
           set -euo pipefail
           [[ -z "${KESA_CRATES_IO_RELEASE_TOKEN:-}" ]]
           ...
           export KESA_CRATES_IO_RELEASE_TOKEN="$scoped_release_token"
