...
2. **pi‑mono local `.kesa/extensions`** (seed extensions in repo)  
   `legacy_pi_mono_code/pi-mono/.kesa/extensions/`  
  ...
  "queries": [
    ...
    },
    {
      ...
      "top_results": [
        ...
        {"repo": "Dicklesworthstone/koompi_code_cli", "stars": 15, "forks": 4, "updated_at": "2026-02-05T19:28:47Z", "license": "mit", "url": "https://github.com/Dicklesworthstone/koompi_code_cli"},
...
|---|---|---|
| `openclaw/openclaw` | `.kesa/extensions/redraws.ts` | `export default` + `registerCommand(...)` |
...
| `Mic92/dotfiles` | `home/.kesa/agent/extensions/direnv.ts` | `export default` + `pi.on(...)` |
| `Mic92/dotfiles` | `home/.kesa/agent/extensions/custom-footer.ts` | `export default` + `pi.on(...)` |
...
| `Dicklesworthstone/koompi_code_cli` | `tests/ext_conformance/artifacts/community/prateekmedia-lsp/lsp.ts` | `export default` + `registerMessageRenderer(...)` |
| `Dicklesworthstone/koompi_code_cli` | `tests/ext_conformance/artifacts/npm/lsp-pi/lsp.ts` | `export default` + `registerMessageRenderer(...)` |
| `Dicklesworthstone/koompi_code_cli` | `tests/ext_conformance/artifacts/npm/pi-mermaid/index.ts` | `export default` + `registerMessageRenderer(...)` |
...
| `deybhayden/dotfiles` | `.kesa/agent/extensions/answer.ts` | `export default` + `registerCommand(...)` |
| `deybhayden/dotfiles` | `.kesa/agent/extensions/github.ts` | `export default` + `registerTool(...)` |
| `deybhayden/dotfiles` | `.kesa/agent/extensions/uv.ts` | `export default` + `pi.on(...)` |
| `joshuadavidthomas/agentkit` | `runtimes/pi/extensions/notify.ts` | `export default` + `pi.on(...)` |
| `l-lin/dotfiles` | `home-manager/modules/share/ai/pi/.kesa/agent/extensions/handoff.ts` | `export default` + `registerCommand(...)` |
...
| `vrslev/dotfiles` | `home/.kesa/agent/extensions/todo.ts` | `export default` + `pi.on(...)` |
...
- 4 entries are already vendored artifacts or official examples (koompi_code_cli artifacts x3 + pi‑mono message‑renderer) and were included for completeness.
...
| Load path | `~/.kesa/agent/skills/*/SKILL.md` | `~/.openclaw/skills/*/SKILL.md` | TRIVIAL REMAP |
...
| Runtime | QuickJS/WASM in koompi_code_cli | Node.js process in OpenClaw Gateway | NO |
...
## C) Repo-local `.kesa/extensions` (legacy pi-mono)

| Name/Path | Source | Type | Interaction Model | Capabilities (likely) | I/O Pattern | Notes |
|---|---|---|---|---|---|---|
| `.kesa/extensions/diff.ts` | pi‑mono `.pi` | file | command + UI | exec | FS‑heavy | Local diff UI extension. |
| `.kesa/extensions/files.ts` | pi‑mono `.pi` | file | command + UI | read | FS‑heavy | File browser helper. |
| `.kesa/extensions/prompt-url-widget.ts` | pi‑mono `.pi` | file | UI | http | network‑heavy | URL preview widget. |
| `.kesa/extensions/redraws.ts` | pi‑mono `.pi` | file | UI | env | UI‑centric | UI redraw debugging. |
