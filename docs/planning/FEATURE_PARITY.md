...
| Compatibility scanner (Node API audit) | ✅ | `src/extensions.rs`, `src/package_manager.rs` | `tests/ext_conformance_artifacts.rs` | Emits compat ledgers when `KESA_EXT_COMPAT_SCAN` is enabled |
...
| Environment variables | ✅ | `src/config.rs` | - | KESA_CONFIG_PATH/KESA_CODING_AGENT_DIR/KESA_PACKAGE_DIR/KESA_SESSIONS_DIR + provider API keys |
