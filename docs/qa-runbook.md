...
`rejected_evidence_cache_entries`. Use `KESA_PERF_EVIDENCE_CACHE_TTL_HOURS` to
...
PERF_EVIDENCE_DIR=tests/perf/reports \
  KESA_GENERATE_PERF_BUDGET_REPORT=1 rch exec -- cargo test --test perf_budgets --profile perf generate_budget_report -- --nocapture
```
...
KESA_GENERATE_BENCH_SCHEMA_DOCS=1 cargo test --test bench_schema generate_schema_doc -- --nocapture
```
...
| **e2e** | Full system with real providers, network, or tmux. | `KESA_E2E=1 cargo test --test e2e_*` |
